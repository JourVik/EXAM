...
vector<string> readPatternsFromFile(const string& filename) {
    ...
    ifstream file(filename);
    if (!file.is_open()) {
        cerr << "Не удалось открыть файл с шаблонами " << filename << endl;
        return patterns;
    }
    string pattern;
...
int main() {
    Grep grep;
    vector<string> filenames = {"File1.txt", "File2.txt"}; // Заменить на имена файлов
    vector<string> patterns = readPatternsFromFile("Shablon.txt"); // Заменить на имя шаблона
